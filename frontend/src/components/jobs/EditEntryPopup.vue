...
const writableEntryValues = ref(
  Object.keys(layout.value).reduce(
    (acc, key: string) => {
      acc[key] =
        props.entryValues[key] || !props.newEntry
          ? props.entryValues[key]
          : getDefaultForType(layout.value[key])
      return acc
    },
...
const getElementForColumn = defineComponent({
  ...
  setup(subprops: any, { emit, slots }: any) {
    const availableTypes = props.canModifySchema
      ? 'string|number|boolean'
      : layout.value[subprops.column]
    const moduleValue = ref(subprops.moduleValue)
    const selectedType = ref<string>(
      typeof subprops.moduleValue !== 'undefined'
        ? typeof subprops.moduleValue
        : layout.value[subprops.column].split('|')[0],
    )
      ...
      switch (selectedType.value) {
            ...
            { variant: 'in', class: 'w-full' },
            {
              default: () => [
                h('label', { for: `${subprops.column}-number`, class: 'z-10' }, [
                  props.readonly || subprops.column === 'id'
                    ? h('span', { class: 'text-warning mr-2' }, 'Readonly')
                    : '',
                  'Number',
                ]),
                h(InputNumber, {
        ...
        case 'boolean':
          return h(
            ...
            {
              class:
                'w-full flex flex-col bg-crust p-2 rounded-lg border-info border-1 hover:border-app color-change-trans \
        ...
        case 'undefined':
          return h(
            'div',
            { class: 'text-warning mr-auto' },
            `Column has no value or type for this entry`,
          )
        default:
          return h(
            'div',
            { class: 'text-warning' },
            `Unknown type: ${availableTypes} for ${subprops.column}`,
          )
      }
    })
...
const submitChanges = () => {
  ...
  const reconstruction: Record<string, any> = {}
      ...
      <div class="w-full mt-4">
        <div class="section-container bg-panel-h rounded-2xl pt-2 pb-1">
          ...
          <div class="editor-box">
            <template
              v-for="key in Object.keys(layout).filter((key) => internalColumns.includes(key))"
              :key="key"
            >
              <getElementForColumn
                :column="key"
                :module-value="writableEntryValues[key]"
        ...
        <div class="section-container bg-panel-h/70 rounded-2xl pt-1 pb-2 mt-2">
          ...
          <div class="editor-box">
            <template v-for="key in externalColumns" :key="key">
        ...
        <div
          v-if="props.canModifySchema && !props.readonly"
          class="flex flex-col items-center w-full mt-6"
        >
          <InputGroup class="mt-2 max-w-128">
            <FloatLabel variant="in">
              <InputText v-model="newColumnName" :disabled="props.readonly" />
