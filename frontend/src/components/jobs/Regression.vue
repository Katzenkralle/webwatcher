...
watch(
    ...
    (newVal) => {
        katex.render(
            newVal,
            katexElement.value as HTMLElement,
            {
                throwOnError: false,
                displayMode: true,
                output: "mathml",
...
<template>
    <div class="flex flex-col gap-2">
        <div class="flex flex-wrap gap-2 justify-between items-center">
            ...
            <FloatLabel variant="in">
                <Select
                    ...
                    :options="dataHandler.computeLayoutUnfiltered.value.filter(col => col.type === 'number').map(col  => col.key)"
                    id="xColumn"
                />
        ...
        <p class="text-text/50">
            The regression is conducted on all rows present in the table. 
            Thus, filter rules will be applied.
        </p>
    </div>
</template>
