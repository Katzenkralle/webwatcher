...
watch(useStatusMessage().getRecentStatusMessage, (last_msg) => {
  ...
  const interval = setInterval(() => {
    const last_index = parseInt(
      Object.keys(displayedMessages.value)[Object.keys(displayedMessages.value).length - 1],
    )
    if (
      isOpendInteractive.value == 'interactivly' ||
      (!isNaN(last_index) && last_index != last_msg.index)
    ) {
      ...
      >
        <div class="flex flex-col h-full w-full p-3 overflow-y-auto overflow-x-hidden">
          ...
          <div class="grid grid-cols-3 gap-y-4 justify-items-stretch">
            <template v-if="Object.keys(displayedMessages).length">
              <template
                v-for="[index, msg] in Object.entries(displayedMessages).reverse()"
                :key="index"
              >
                <span class="grid_element col-span-2">
                  <Message class="border border-2 w-full h-full" :severity="msg.severity">
                    <template #icon>
                ...
                <span class="grid_element">
                  <Button
                    class="h-full w-fit text-center ml-2 animate-fade-in"
                    label="Remove"
                    @click="
                      () => {
          ...
          <div
            :class="{
              'w-full h-[4px] rounded-lg relative mt-4 bg-crust-d': true,
