  ...
  <div v-if="graphs && graphs?.length > 0" class="flex flex-col w-full items-center">
    ...
    <div class="flex flex-wrap w-[98%] gap-2 justify-center">
      <div
        ...
        :key="graph.name"
        class="flex flex-col gap-2 w-full max-w-200 p-2 bg-crust border-2 rounded-lg border-solid border-h-text-h"
      >
        <InputGroup>
              ...
              @click="
                () => {
                  ...
                  props.userConfig.graph.value = {
                    change: {
                      index: index,
                      value: { name: changedGraphNames[index], data: graph.data },
                    },
                  }
                }
              "
          ...
          <template v-else>
            <ConfirmableButton
              confirm-message="Are you sure you want to delete this graph?"
